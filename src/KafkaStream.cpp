...
void
erskafka::KafkaStream::ers_to_json(const ers::Issue& issue, size_t chain, std::vector<nlohmann::json>& j_objs)
{
  try {
    ...
    message["usecs_since_epoch"] =
      std::chrono::duration_cast<std::chrono::microseconds>(issue.ptime().time_since_epoch()).count();
...
void
erskafka::KafkaStream::kafka_exporter(std::string input, std::string topic)
{
  try {
 ...
 */
void
erskafka::KafkaStream::write(const ers::Issue& issue)
{
  try {
    ...
    tmpstream << issue.context().process_id() << issue.time_t() << issue.context().application_name()
              << issue.context().host_name() << rand();
